    ...
    return (
      ...
      >
            ...
            {selected.map((option) => {
              return (
                <Badge
                  key={option.value}
                  className={cn(
                    'data-[disabled]:bg-muted-foreground data-[disabled]:hover:bg-muted-foreground bg-custom-3 data-[disabled]:text-muted',
        ...
        <div className='relative'>
          {open && (
            <CommandList className='absolute top-1 z-10 w-full rounded-md border bg-popover text-popover-foreground shadow-md outline-none animate-in dark:bg-black bg-white'>
              {isLoading ? (
                <>{loadingIndicator}</>
              ) : (
                <>
